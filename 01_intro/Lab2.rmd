...
<li><b> The distribution is right skewed. </b></li> 
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(med = median(arr_delay), iqr_range = IQR(arr_delay))
```
...
nycflights %>% group_by(month) %>% summarise(avg = mean(dep_delay)) %>% arrange(avg)
```
...
nycflights %>% group_by(month) %>% summarise(med = median(dep_delay)) %>% arrange(desc(med))
```
...
<ol>
<li> <b> Mean would be more reliable as it gives us the true average. </b> </li> 
...
nycflights <- nycflights %>% mutate(dep_status = ifelse(dep_delay > 5, "DELAYED", "ON TIME"))
nycflights %>% group_by(origin) %>% summarise(ot_dep_rate = sum(dep_status=="ON TIME")/n()) %>% arrange(ot_dep_rate)
```
...
<b>.5887582, or about 59% arrived on time</b>

```{r on-time-arr-perc}
# type your code for Question 10 here, and Knit
nycflights <- nycflights %>% mutate(arr_status = ifelse(arr_delay<=0, "ON TIME", "DELAYED"))
nycflights %>% summarise(on_time_arr = sum(arr_status=="ON TIME")/n())
```